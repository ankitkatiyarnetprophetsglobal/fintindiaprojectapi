...
use App\Models\Activitymeasurements;
use Response;
use Helper;
...
class ChallengermastersController extends Controller
{
    public function __construct() {
        ...
        $this->middleware('auth:api', ['except' => ['challenger_masters','challenger_masters_id','store_challenger_user','store_challenger_data','get_challenger_data','get_point_generate','get_challenger_data_weekly','getactivitymeasurement']]);
    ...
    public function store_challenger_data(Request $request){
        ...
        //     DB::table('serverhittables')->insert(
        ...
        try{
            ...
            if($user){
                ...
                $data = Challengermasters::
                                        ...
                                        // ->whereDate('challenger_masters.start_time', '<=', $start_date_format)
                                        // ->whereDate('challenger_masters.end_time', '>=', $curent_date)
                                        ->select(
                                            ...
                                            DB::raw('DATE(end_time) as cm_end_time'),
                ...
                if($data->count() > 0){
                    ...
                    foreach ($data as $value) {
                        ...
                        $challenger_mastersid = $value['challenger_mastersid'];
                        ...
                        switch ($value['sport_type']) {
                            ...
                            case 'w':
                                ...
                                if($banner_type == 'd'){
                                    // dd($start_date_format);
                                    $this->daylycalculation($challenger_mastersid,$start_date_format,$duration,$cm_start_time,$cm_end_time,$goal,$oc_reward,$oc_id);
                                }

                                if($banner_type == 'w' || $banner_type == 'm'){

                                    $this->weeklymonthlyculation($challenger_mastersid,$duration,$cm_start_time,$cm_end_time,$goal,$oc_reward,$oc_id);
                                }
                            ...
                            case 'r':
                                ...
                                if($banner_type == 'd'){
                                    ...
                                    $this->daylykmcalculation($challenger_mastersid,$start_date_format,$duration,$cm_start_time,$cm_end_time,$goal,$oc_reward,$oc_id);
                                }
    ...
    function daylycalculation($challenger_mastersid,$start_date_format,$duration,$cm_start_time,$cm_end_time,$goal,$oc_reward,$oc_id){
        ...
        $data_calculation = Storechallengerdatas::
                                                Join('challenger_masters', 'challenger_masters.id', '=', 'storechallengerdatas.challenger_mastersid')
                                                ->where([
                                                        ['storechallengerdatas.status','=',1],
                                                    ])
                                                // ->whereDate('storechallengerdatas.date', '=', $start_date_format)
                                                ->whereDate('challenger_masters.start_time', '>=', $cm_start_time)
                                                ->whereDate('challenger_masters.end_time', '<=', $start_date_format)
                                                // ->whereDate('challenger_masters.end_time', '>=', $start_date_format)
                                                ->whereDate('storechallengerdatas.date', '>=', $cm_start_time)
                                                ->whereDate('storechallengerdatas.date', '<=', $start_date_format)
                                                ->where("storechallengerdatas.challenger_mastersid","=",$challenger_mastersid)
    ...
    function weeklymonthlyculation($challenger_mastersid,$duration,$cm_start_time,$cm_end_time,$goal,$oc_reward,$oc_id){
        ...
        // dd(1);
        // if($date_cal_format == $curent_date){
        if($date_cal_format == $cm_end_time){
    ...
    function daylykmcalculation($challenger_mastersid,$start_date_format,$duration,$cm_start_time,$cm_end_time,$goal,$oc_reward,$oc_id){
    ...
    public function getactivitymeasurement(Request $request){

            // $measurement = $request->measurement;
            // if($measurement == null || $measurement == '' || is_int($request->measurement) === false){

            //         $error_code = '801';
            //         $error_message = 'Required Measurement Unit';
            ...
            if($user){
                    ...
                    $data = Activitymeasurements::
                        where([
                                // ['activitymeasurements.id','=',$measurement],
                                ['activitymeasurements.status','=' , 1],
