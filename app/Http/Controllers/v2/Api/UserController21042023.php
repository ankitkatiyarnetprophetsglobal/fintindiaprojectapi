...
class UserController extends Controller
{
	...
	public function __construct() {
        $this->middleware('auth:api', ['except' => ['login', 'logintest', 'store', 'storenew', 'check','getchallenge','updateDeleteChallengeRow','socialregistraton', 'mobileregistration','elogin','mlogin']]);
    }
	...
	function elogin(Request $request){
		...
		try{ 
			...
			if(($email == '' || $deviceid == '' || $FCMToken == '' || $medium == '' || $authid == '') && $medium != 'apple'){
				return Response::json(array(
					'status'    => 'error',
					'code'      =>  422,
			...
			if(is_numeric($email)){
				...
				$validator = Validator::make( array("authid" => $authid, "role" => $request->role ), [
					'authid' => 'required',
					'role' => 'required|string|min:6',
				]);
			}else{
...
// 			// only for apple not given email
// 			if($medium != 'apple' && $email == ''){
// 				if($deviceid == '' || $FCMToken == '' || $authid == '' || $email == ''){
// 					return Response::json(array(
// 						'status'    => 'error',
// 						'code'      =>  422,
// 						'message'   =>  "Required Filed"
...
// 			}else if($medium == 'apple' && $email == false){
// // dd(987);
// 					$validator = Validator::make(array("email" => $email, "password" => $password, "role"=> $request->role), [
