...
class ChallengesController extends Controller
{
    ...
    public function getuserhistorylist(Request $request){
        try{
            ...
            // $data = Userhistorytraking::with(['getMasterGroupDetails'])->where('status', 1)->where('user_id', '=', $request->user_id)->take(20)->orderBy('id', 'DESC')->get();
            $data = Userhistorytraking::with(['getMasterGroupDetails'])->where('status', 1)->where('user_id', '=', $request->user_id)->orderBy('id', 'DESC')->get();
    ...
    public function getusercarbonsave(Request $request){
        try{

            $user_id = $request->user_id;
            $date = $request->date;
            $modegroupid = $request->modegroupid;

            if($user_id == '' || is_int($request->user_id) === false){
            ...
            if($modegroupid != ""){
                ...
                // $userhistory_count = Userhistorytraking::with(['getMasterGroupDetails'])->where('status', 1)->where('user_id', '=', $request->user_id)->orderBy('id', 'DESC')->get();
                $userhistory_count = Userhistorytraking::with(['getMasterGroupDetails'])
                                                        ->where([['user_id', $user_id],['modegroupid','=',$modegroupid],['status','=',1]])
                                                        ->orderBy('id', 'DESC')->count();


                // $userhistory_count = DB::table('userhistorytrakings')
                //                                     ->where([['user_id', $user_id],['modegroupid','=',$modegroupid],['status','=',1]])
                //                                     ->count();

                $data_carbon = Userhistorytraking::
                                        with(['getMasterGroupDetails'])
                                        ->select(DB::raw('IFNULL(SUM(carbonSave), 0) as carbon'),DB::raw('IFNULL(SUM(distance), 0) as distance'))
                                        ->where([['user_id', $user_id],['modegroupid','=',$modegroupid],['status','=',1]])
                                        ->first();
            }else{
                ...
                if(isset($date)){
                    ...
                    $data_carbon = Userhistorytraking::with(['getMasterGroupDetails'])
                                            ->select(DB::raw('IFNULL(SUM(carbonSave), 0) as carbon'),DB::raw('IFNULL(SUM(distance), 0) as distance'))
                                            ->where('user_id', $user_id)
                                            ->where('status','=',1)
                                            ->whereDate('datetime', $date)
            ...
            $controller_name = 'ChallengesController';
            $function_name = 'getusercarbonsave';
            $error_code = '901';
            $error_message = $e->getMessage();
            $send_payload = json_encode($request->all());
            $response = null;
